...
function setup({micError,requestError,saveError,pollError,emptySpeech,formatError}={}) {
  const elements=new Map(), storage=new Map([['vrx_worker_url','https://example.test'],['vrx_token','fictional-test-token']]);
  ...
  const context=vm.createContext({console:{log(){},error(){},warn(){}},Blob,URL,URLSearchParams,AbortController,crypto:require('node:crypto').webcrypto,MediaRecorder:Recorder,
    ...
    fetch:async(url,options)=>{calls.push(url);if(requestError&&url.includes('/transcribe'))throw Error('Network unavailable');if(pollError&&url.includes('/poll/'))throw Error('Network unavailable');return {ok:!(formatError&&url.includes('/format')),status:formatError?500:200,json:async()=>url.includes('/transcribe')?{id:'fictional-job'}:url.includes('/poll/')?{status:'completed',text:emptySpeech?'':'Fictional encounter transcript.'}:url.includes('/format')?{note:'Subjective: Fictional encounter.\nAssessment: Test only.'}:{}};},
